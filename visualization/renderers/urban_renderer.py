...
class UrbanRenderer:
    """
    Reusable renderer family for urban and built-up indices (NDBI, UI, EBBI).
    Colors range from White (Non-Urban) through Light Yellow, Brown, to Dark Gray (High Urban Density).
    """
    
    @staticmethod
    def _create_ramp() -> 'QgsGradientColorRamp':
        colors = ["#FFFFFF", "#FFFDE7", "#FFD54F", "#8D6E63", "#37474F"]
    ...
    def create(provider, min_val: float, max_val: float) -> 'QgsSingleBandPseudoColorRenderer':
        ...
        ramp = UrbanRenderer._create_ramp()
